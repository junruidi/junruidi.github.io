...
- [CRAN Task View: Optimization and Mathematical Programing](https://cran.r-project.org/web/views/Optimization.html)
...
Consider the data generating model that generate $x = \{y, z \}$, where $x$ is the full data, $y$ is the observed data, and $z$ is the latent data/missing values, and parameters $\theta$
...
E Step: Calculate the expected value of the compete log likelihood with respect to the condition distribution of $z$ given $y$, under the $\theta^{(k)}$ from the current iteration $k$.
...
- The ascent property of EM: for the observed ata likelihood $L(\theta^{(k+1)}) \geq L(\theta^{(k)})$ 
...
$$z_{ij} = \begin{cases}1 \quad \text{if } y_i \text{ comes of the th component} \\ 0 \quad \text{otherwise}\end{cases}$$
...
We need to consider other approach for the estimation, e.g. ridge regression.

---

<center> <h3> Ridge Regression </h3> </center>

#### 1. Ridge via pennalization v.s. Ridge via constraints

$$l(\beta) = ||y - X\beta||^2_2 +\lambda ||\beta||^2_2$$
which is a convex function (both parts are convect), and
...
#### 1. LASSO via pennalization v.s. Ridge via constraints
...
#### 3. Solution for orthonormal desgin ($X'X = nI_p$)
...
Or, the region wihtin the function is also a convex set. (Remeber it as a cup with minimum).

#### 2. Properties

1. $f''(x)>0$.
2. Any norm is a convex function.
3. Nonnegative weighted sum of convex function is also convex.
4. If $f_1,f_2, \dots, f_n$ are all convext, then $\text{max}_n\{f_1(x), \dots, f_n(x)\}$ is also convex.

#### 3. Check for convexity

1. First order defnition 

$$f(x)\text{ is convext } \iff f(y)\geq f(x)+\nabla f(x)^T (y-x) \quad \forall x,y\in\mathbb{R}^p$$
...
Property 1. $-g(\lambda, v)$ is convext

Property 2. If we define $f^{*} = \text{inf}_{\text{feasible } x}f(x)$, then $\forall \lambda \geq 0$ and $\forall v$, $g(\lambda, v)\leq f^{*}$. 

#### 6. KKT conditions
Now all assume $f$, $g$, $h$ are all differentiable and convext, then if $x^{*}$ and $(\lambda^*, v^*)$ are primal and optioal, we have the following conditions

1. $\nabla f(x^*)+\sum_{k}\lambda_j^*\nabla g_j(x^*)+\sum_k v_k^* \nabla h_k(x^*) =0$
2. $x^*$ is feasible, i.e. $g_j(x^*)\leq 0$ and $h_k(x^*)=0$
3. $\lambda_j \geq 0$
4. $\sum_j \lambda_j^* g_j(x^*) = 0$, then $\forall j$, either $\lambda_j^* = 0$ or $g_j = 0$.
